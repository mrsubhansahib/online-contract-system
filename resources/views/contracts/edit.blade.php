        ...
        <div class="col-md-11 mx-auto">
            ...
            <div class="card">
                ...
                <div class="card-body py-3 pb-5">
                    ...
                    <form action="{{ route('contract.update') }}" method="POST" id="form-builder-pages"
                            ...
                            <select name="category" id="category" class="form-control" required>
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ $category->id == $contract->category_id ? 'selected' : '' }}>{{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('category')
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <input type="text" id="title" name="title" placeholder="Enter Contract Title"
                                value="{{ $contract->title }}" class="form-control" required>
                            @error('title')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                            ...
                            <select name="users[]" id="users" class="form-control select2" multiple required>
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ $contract->contract_users->where('user_id', $user->id)->count() ? 'selected' : '' }}>
                                        {{ $user->name }}</option>
                                @endforeach
                            </select>
                            @error('users')
            ...
            jQuery(($) => {
                ...
                var mainInterval = setInterval(() => {
                    ...
                    if (count == 0) {
                        ...
                        // console.log($newTab.querySelector("a"));
                    }
                    const $tabLink = $newTab.querySelector("a");
                    $tabLink.setAttribute("href", "#" + tabId);
                    $tabLink.innerText = "Page " + (tabCount);
